    ...
    def get(self):
        ...
        if self.error == '' and storeitem is not None:
            ...
            if item['dependencies'] != '':                            # check if the item has dependencies
                ...
                if depc >= len(
                        deps):                                        # alter all counting, if counter is more than all dependecies, it probably means user has all dependencies
            ...
            if add == True and item['maximum'] != '':                        # check if the item has maximum limitation
                ...
                for myitem in myitems:                                        # run through myitems
                    if myitem.itid == item['id']:                            # check if user has same item in hand
                        depc = depc + 1                                        # increment counter
                if int(depc) >= int(
                        item['maximum']):                        # if counter is more than or equal the item's maximum
            ...
            if add == True:
                if player.state_obj['gold'] >= item['gold']:                # check if user has gold enough to purchase
                    ...
                    itemobj = Item(parent=db.Key.from_path('Item', config.db[
                        'itemdb_name']))    # now create an item and reward/deliver to user
                    ...
                    itemobj.timestamp = time.time() + float(
                        item[
                            'time'])        # calculate time to deliver or reward, while status pending, user can't use it
                    if itemobj.put():                                        # put it into database
                        if player.state_obj['token'] != '':                # check if user already has token
                            apns.add(player.state_obj['token'],
                                     storeitem[str(itemobj.itid)]['title'] + ' has been delivered to you!',
                                     itemobj.timestamp) # if so, set a push notofication to be sent
                        if Player.setplayer_as_obj(self, player):            # put player state back into the database
                            myitems = Item.getspecificitems(self, uuid, item[
                                'id'])    # get user items (only same type of purchased item), we want to list them all and return
                                ...
                                for myitem in myitems:                        # run through myitems
                                    if storeitem[str(myitem.itid)]:            # check if item does exist
                                        ...
                                        if myitem.status == 'pending' and time.time() >= myitem.timestamp:                                    # if item status is pending and it is time to reward
                                            ...
                                            myitem.timestamp = time.time() + storeitem[str(myitem.itid)][
                                                'produce_time']                    # calculate next time to produce resource
                                        ...
                                        self.respn += '"desc"		: "' + storeitem[str(myitem.itid)][
                                            'description'] + '",'
                                        self.respn += '"imgurl"		: "' + storeitem[str(myitem.itid)][
                                            'image_url_sd'] + '",'
                                        self.respn += '"status"		: "' + myitem.status + '",'
                                        self.respn += '"timestamp"		: ' + str(myitem.timestamp)
                                        self.respn += '},'

                                        if save == True:                                                                                    # now we should know, should we update item data
